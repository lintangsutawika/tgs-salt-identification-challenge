...
model.load_state_dict(torch.load('model_checkpoint_fold0.pth'))
#############################################################################################################
fold_score = []
for cv_fold, (train_idx, valid_idx) in enumerate(sss.split(SaltLevel['train_ids'], SaltLevel['salt_class'])):    
    model.train()
    ...
    for e in range(epoch):
        ...
        if validation_iou > best_iou:
            best_iou = validation_iou
            torch.save(model.state_dict(), "model_checkpoint_finetune_fold{}.pth".format(cv_fold))
            fold_score[cv_fold] = best_iou
            print("Better validation, model saved")
        else:
            pass
...
model.load_state_dict(torch.load('model_checkpoint_finetune_fold0.pth'))
model.eval()
y_pred_true_pairs = []
img_list = []
with tqdm(val_loader) as pbar:
