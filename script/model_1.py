...
X_train = np.zeros((len(train_depth.id.values), 299, 299, 3), dtype=np.uint8)
...
for n, id_ in tqdm_notebook(enumerate(train_depth.id.values), total=len(train_depth.id.values)):
    ...
    canny = cv2.Canny(np.asarray(img),100,200)
    ret,img_canny = cv2.threshold(canny,90,255,cv2.THRESH_BINARY)
    img = cv2.medianBlur(np.asarray(img),5)
    ret,img_med = cv2.threshold(img,90,255,cv2.THRESH_BINARY)

    x = resize(x, (299, 299, 1), mode='constant', preserve_range=True)
    img_canny = resize(img_canny, (299, 299, 1), mode='constant', preserve_range=True)
    img_med = resize(img_med, (299, 299, 1), mode='constant', preserve_range=True)
    x = np.dstack((x,img_canny,img_med))
    X_train[n] = x
print('Done!')
...
model_saltClass.fit(X_train, Y_train, validation_split=0.1, epochs=48, batch_size=16, verbose=1)
